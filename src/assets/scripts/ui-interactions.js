...
const init = () => {
	...
	//go projects section page and scrollIntoView into the last clicked project
	function goToProjectsSection() {
		if (window.location.pathname == "/") {
			const projectLinks = document.querySelectorAll("#projects-section > div > a");
			const lastProjectIndex = window.sessionStorage.getItem("lastProjectIndex");
			let targetProject = projectLinks[0]; // default to first
			if (lastProjectIndex !== null && projectLinks[lastProjectIndex]) {
				targetProject = projectLinks[lastProjectIndex];
				window.sessionStorage.removeItem("lastProjectIndex");
			}
	...
	if (window.location.pathname.includes("/projects/")) {
		// Save the last clicked project index when clicking a project link on the home page
		// (This should be set on the home page, not here)
		if (projectBackBtn) {
			projectBackBtn.addEventListener("click", () => {
				window.sessionStorage.setItem("projects-section", true);
			});
		}
	}

	// HOME PAGE: Save last clicked project index when clicking a project link
	if (window.location.pathname == "/") {
		const projectLinks = document.querySelectorAll("#projects-section > div > a");
		projectLinks.forEach((projectLink, idx) => {
			projectLink.addEventListener("click", () => {
				window.sessionStorage.setItem("lastProjectIndex", idx);
			});
		});
	}
